...
namespace Sporty.Data
{
    public static class DbInitializer
    {
        public static void Initialize(SportyContext context)
        {
            ...
            var alex_alonso_li_norm = new Event {
                Date = DateTime.Parse("2021-09-13"),
                Time = DateTime.Parse("15:00"),
                Duration = 60,
                Title = string.Empty,
                Members = new List<User>{alexander, alonso, li, norman}
            };

            var group1 = new Event {
                Date = DateTime.Parse("2021-09-13"),
                Time = DateTime.Parse("15:00"),
                Duration = 60,
                Title = string.Empty,
                Members = new List<User>{alexander, anand, li, norman, barzdukas}
            };

            var group2 = new Event {
                Date = DateTime.Parse("2021-09-13"),
                Time = DateTime.Parse("15:00"),
                Duration = 60,
                Title = string.Empty,
                Members = new List<User>{alonso, justice, li, norman, barzdukas}
            };

            context.Events.AddRange(new[] {alex_vs_alonso, alex_vs_li, alex_alonso_li_norm, group1, group2});
            context.SaveChanges();
        }
    }
}
